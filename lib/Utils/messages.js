...
const generateWAMessageContent = async (message, options) => {
    ...
    // =====================================================================
    // PATCH: mueve forwardedNewsletterMessageInfo al nivel raíz del mensaje
    // para que sea visible para TODOS los participantes, no solo el emisor.
    // =====================================================================
    if ('contextInfo' in message && !!message.contextInfo) {
        const [messageType] = Object.keys(m);
        m[messageType] = m[messageType] || {};
        const { forwardedNewsletterMessageInfo, isForwarded, forwardingScore, ...cleanContextInfo } = message.contextInfo;
        m[messageType].contextInfo = cleanContextInfo;
        if (forwardedNewsletterMessageInfo) {
            // Va al nivel raíz del mensaje — aquí es donde WA lo espera
            m.forwardedNewsletterMessageInfo = forwardedNewsletterMessageInfo;
        }
        if (isForwarded !== undefined) {
            m[messageType].contextInfo.isForwarded = isForwarded;
        }
        if (forwardingScore !== undefined) {
            m[messageType].contextInfo.forwardingScore = forwardingScore;
        }
    }
    // =====================================================================
